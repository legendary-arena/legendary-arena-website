...
| ID | Title | Status | Effort | Link |
|---|---|---|---|---|
| WP-039 | Universal Rulebook page (`/rules`) + registry-viewer cross-reference | ✅ Done (marketing) — `/rules` live with anchor contract published; the registry-viewer glossary cross-link is an **optional engine-repo follow-on** (`apps/registry-viewer`, ~15 min, not a blocker) | ~1 day | [WP-039](work-packets/WP-039-universal-rulebook-page.md) |
| WP-040 | Rename blog section URL `/posts/` → `/blog/` | ✅ Done — renamed and live; the CF zone Redirect Rule was **consciously skipped** (no inbound `/posts/` links to catch, per Jeff 2026-08-26) | ~half-day | [WP-040](work-packets/WP-040-blog-url-rename.md) |
...
| WP-045 | Interactive voting on the roadmap board (`/roadmap/`) — one-click upvote/un-vote per enhancement for signed-in visitors, via the engine's authenticated `POST`/`DELETE /api/feedback/:id/vote` (engine WP-604 / D-24414). Reuses the proven `hanko` Bearer-token auth from `header-auth.js` (no cookies/`credentials`) | ✅ Done (2026-08-26) — live on `/roadmap/`; hard-dep CORS verified (POST/DELETE + Authorization from `www`); optimistic vote/un-vote + 401 re-auth + signed-out sign-in link; voting behavior verified in-browser against the built JS via a mocked API (live board empty until operator triage); zero console errors; Perf unregressed | ~half-day | [WP-045](work-packets/WP-045-interactive-voting.md) |
| WP-046 | Lighthouse-on-CI performance gate — makes the ≥90 P/A/BP/SEO invariant an automated PR check (median-of-3 Lighthouse over the 7-page matrix; compressed server; API-backed pages Perf-only). Closes the "silent regression" gap WP-043 exposed | ✅ Done (2026-08-26) — gate live in `.github/workflows/lighthouse.yml`; **both paths proven on CI**: green (21 runs pass on the WP-044 baseline) and red (a severe blocking regression failed the gate with `performance minScore` violations on all 7 pages); advisory until marked required | ~half-day | [WP-046](work-packets/WP-046-lighthouse-ci-gate.md) |
...
| WP-045 | WP | Interactive voting on the roadmap board | ✅ Done | P2 | ~half-day | Claude | WP-043; engine WP-604 |
| WP-046 | WP | Lighthouse-on-CI performance gate | ✅ Done | P2 | ~half-day | Claude | WP-044 (baseline to protect) |
